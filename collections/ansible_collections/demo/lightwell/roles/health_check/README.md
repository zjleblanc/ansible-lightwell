...
| --- | --- | --- |
| `app_container_name` | `lightwell-patch-demo-app` (`lightwell-patch-demo-app-dev` for `dev`) | Name of the container to check. Defaulted here (rather than only in `deploy_app`) so this role works standalone, e.g. when included from `rollback`. |
| `app_host_port` | `8080` if `dev`, `8081` otherwise | Host port the application is bound to, derived from `app_environment`. |
